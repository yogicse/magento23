...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function _getCartInformation($addressId, $countryId, $setRegionId, $shipping_method, $zipcode, $currencyCode)
    {
        ...
        //$cartInfo['cart_items_count']     = (float)$this->cartHelper->getSummaryCount();
        $cartInfo['cart_items_count']     = $cart->getItemsCount();//(float)$this->cartHelper->getSummaryCount();
        ...
        $cartInfo['reward_amount_applied'] = $cart->getQuote()->getRewardAmount()? $cart->getQuote()->getRewardAmount(): "";
    ...
    {
        return $this->currentCurrency->load($currencyCode)->getCurrencySymbol() ? $this->currentCurrency->load($currencyCode)->getCurrencySymbol() : $currencyCode;
    }
    ...
    protected function _getCartItems($currencyCode)
    {
        $cartItemsArr            = array();
        ...
        foreach ($items as $item) {
            ...
            $cartItemArr['currency']          = $this->currentCurrency->load($currencyCode)->getCurrencySymbol()?
            $this->currentCurrency->load($currencyCode)->getCurrencySymbol() : $currencyCode;
    ...
    protected function _getAdditionalInfo($item)
    {
        $options = $item->getProduct()->getTypeInstance(true)->getOrderOptions($item->getProduct());
    ...
    public function loadParent($helper)
    {
        ...
        if ($this->scopeConfig->getValue('Smyapp/secure/token') != $helper) {
            echo json_encode(array('status' => 'error', 'code' => '002'));
            exit;
        }
        if (!$this->scopeConfig->getValue('Smyapp/key/status')) {
            echo json_encode(array('status' => 'error', 'code' => '003'));
            exit;
        }
        if ($this->compareExp() > 4800 ||
            $this->scopeConfig->getValue('Smyapp/secure/token') != isset($helper)
            || !$this->scopeConfig->getValue('Smyapp/key/status') || !$helper) {
    ...
    public function getOrderDetails($_orderId, $currencyCode)
    {
        if ($_orderId) {
            ...
            if ($data_order) {
                ...
                $data_order['symbol']    =$this->currentCurrency->load($currencyCode)->getCurrencySymbol()?$this->currentCurrency->load($currencyCode)->getCurrencySymbol():$currencyCode;
                $result                  = array();
    ...
    public function getSpecialPriceProduct($product)
    {
        ...
        print_r($this->getSpecialPriceByProductId($product));
        echo "product id";print_r($product->getId());
        echo "</br>";
        //die('hello');   
        $specialprice         = $this->getSpecialPriceByProductId($product);
        //$final_price_with_tax = $product->getFinalPrice();
        return $specialprice;
    ...
    public function getSpecialPriceByProductId($product)
    {
       // $product              = $this->productModel->load($productId);
        ...
        $today = time();
//        echo "product id";print_r($product->getId());
//        print_r($product->getData('special_from_date'));
//        print_r($product->getData('special_to_date'));
//        echo "special price";print_r($product->getData('special_price'));
//        echo "</br>";
        //die('sfsd');
        if ($specialprice) {
