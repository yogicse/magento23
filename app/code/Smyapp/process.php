...
include_once("paypal.class.php");
	...
	if(_GET('paypal')=='checkout'){
		...
		$result = [];
		for ($i=0; $i < count($final['itemname_']); $i++) { 

			
			
		$products[$i]['ItemName'] = $final['itemname_'][$i]; //Item Name
		$products[$i]['ItemPrice'] = $final['itemprice_'][$i]; //Item Price
		$products[$i]['ItemNumber'] = $final['itemnumber_'][$i]; //Item Number
		$products[$i]['ItemDesc'] = $final['itemdesc_'][$i]; //Item Number
		$products[$i]['ItemQty']	= $final['itemQty_'][$i]; // Item Quantity
