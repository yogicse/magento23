		...
		function DoExpressCheckoutPayment(){
			...
			if(!empty(_SESSION('ppl_products'))&&!empty(_SESSION('ppl_charges'))){
				...
				if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])){
					...
					$_SESSION['transcation'] = $httpParsedResponseAr; 
					...
					*/
					...
					header("Location: http://localhost:8080/rajnkant/peoplemarket/code/mpaypal/index/failure");
...
$baseurl= $store->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_WEB);
			...
			$tableName = $resource->getTableName('paypal'); //gives table name with prefix
			...
			CORRELATIONID,
			TRANSACTIONID,
			CHECKOUTSTATUS,TOKEN) Values ('".$httpParsedResponseAr["PAYERID"]."','".$httpParsedResponseAr["EMAIL"]."','".$httpParsedResponseAr["PAYERSTATUS"]."','".$httpParsedResponseAr["ADDRESSSTATUS"]."','".$httpParsedResponseAr["CORRELATIONID"]."','".$httpParsedResponseAr["TRANSACTIONID"]."','".$httpParsedResponseAr["CHECKOUTSTATUS"]."','".$httpParsedResponseAr["TOKEN"]."')";
			$connection->query($sql);


			header("Location: ".$baseurl."mpaypal/index/success?&TOKEN=".$httpParsedResponseAr["TRANSACTIONID"]."");

			} 
			else  {

				header("Location: ".$baseurl."mpaypal/index/failure");
